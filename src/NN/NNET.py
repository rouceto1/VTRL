...
def get_dataset(data_path, GT):
    if "nordland" in data_path:
        ...
        if TRAINING > 0:
            ...
            crp = CROP_SIZE2
        dataset = Strands(crp, FRACTION, SMOOTHNESS, GT,
                          thre=TRAINING)  # TODO: threshold does not work abd is currently
...
def eval_displacement(eval_model, data_path, GT, weights_file):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(train_loader):
            ...
            # pdb.set_trace()
            f = interpolate.interp1d(np.linspace(0, WIDTH, OUTPUT_SIZE - 1), shift_hist,
