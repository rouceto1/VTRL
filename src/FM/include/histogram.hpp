...
int internalHistogram(std::vector<cv::KeyPoint> keypoints1,std::vector<cv::KeyPoint> keypoints2, float &displacement ,  int numBins, int (&histogram)[63], std::vector<int> &bestHistogram , std::vector<cv::DMatch> matches, int granularity, int verticaLimit);
