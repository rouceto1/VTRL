...
int main(int argc, char ** argv) 
{
	...
	do{
		...
		tmpIm =  imread(filename, IMREAD_COLOR);
		if (tmpIm.data != NULL)
		{
	...
	for (int location = 0;location<numLocations;location++){
		for (int i=0;i<numSeasons;i++){
			...
			im[i] =  imread(filename,IMREAD_COLOR);
		...
		//Ptr<features2d::StarDetector> detector = cv::features2d::StarDetector::create(45,detectorThreshold,10,8,5);		//TODO make this selectable
		Ptr<FeatureDetector> detector = FastFeatureDetector::create(0,true); 
		...
		for (int ik = 0;ik<numSeasons;ik++){
			for (int jk = ik+1;jk<numSeasons;jk++){
				...
				if (draw&&matchFail)
				{
					...
					drawMatches(imA, kpA, imB, kpB, inliers_matches, img_matches, Scalar(0, 0, 255), Scalar(0, 0, 255), vector<char>());
